...
def _count_requested_attributes(structured_query: dict[str, Any]) -> int:
    """Count how many concrete attributes the user requested."""
    count = 0

    if structured_query.get("product_type"):
        count += 1

    count += len(structured_query.get("colors") or [])

    for key in ["pattern", "collar", "sleeve", "material", "fit", "style"]:
        if structured_query.get(key):
            count += 1

    count += len(structured_query.get("constraints") or [])
    return count


def _evaluate_top_products(products: list[dict], structured_query: dict[str, Any], display_top_k: int) -> dict[str, Any]:
    """
    Step 7 quality gate.

    This is a lightweight self-check over the final reranked products. It does
    not call another model yet. It checks whether the top returned products are
    good enough based on category match, attribute coverage, constraint
    violations, multimodal support, and candidate count.
    """
    requested_attribute_count = _count_requested_attributes(structured_query)
    requested_product_type = structured_query.get("product_type")
    ...
    if not products:
        return {
            ...
            "reason": "no_products_returned",
            "returned_count": 0,
            "requested_attribute_count": requested_attribute_count,
        }

    top_products = products[: max(1, min(display_top_k, len(products)))]
    returned_count = len(top_products)

    category_matches = 0
    total_attribute_matches = 0
    total_attribute_misses = 0
    total_constraint_penalties = 0
    both_path_matches = 0

    for product in top_products:
        if product.get("category_match"):
            category_matches += 1

        total_attribute_matches += len(product.get("attribute_matches") or [])
        total_attribute_misses += len(product.get("attribute_misses") or [])
        total_constraint_penalties += len(product.get("constraint_penalties") or [])

        if product.get("matched_image_path") and product.get("matched_text_path"):
            both_path_matches += 1

    coverage_denominator = max(1, requested_attribute_count * returned_count)
    attribute_coverage = total_attribute_matches / coverage_denominator
    category_rate = category_matches / returned_count if requested_product_type else 1.0
    both_path_rate = both_path_matches / returned_count
    count_rate = min(returned_count / max(1, display_top_k), 1.0)

    miss_penalty = min(total_attribute_misses / coverage_denominator, 1.0) * 0.25
    constraint_penalty = min(total_constraint_penalties * 0.20, 0.50)

    quality_score = (
        0.35 * category_rate
        + 0.35 * min(attribute_coverage, 1.0)
        + 0.15 * both_path_rate
        + 0.15 * count_rate
        - miss_penalty
        - constraint_penalty
    ...
    reasons = []
    if returned_count < max(3, min(display_top_k, 4)):
        reasons.append("too_few_products")
    if requested_product_type and category_rate < 0.50:
        reasons.append("low_category_match")
    if requested_attribute_count > 0 and attribute_coverage < 0.25:
        reasons.append("low_attribute_coverage")
    if total_constraint_penalties > 0:
        reasons.append("constraint_violation_in_top_results")

    is_good_enough = quality_score >= 0.45 and not reasons

    if quality_score >= 0.70 and not reasons:
        quality_label = "good"
    elif is_good_enough:
    ...
    return {
        ...
        "reason": ",".join(reasons) if reasons else "passed_quality_gate",
        "returned_count": returned_count,
        "requested_attribute_count": requested_attribute_count,
        "category_rate": round(category_rate, 6),
        "attribute_coverage": round(attribute_coverage, 6),
        "both_path_rate": round(both_path_rate, 6),
        "constraint_penalty_count": total_constraint_penalties,
    }


def evaluate_result_quality_node(state: FashionState) -> FashionState:
    """
    Step 7: self-check after reranking.

    If quality is poor and retry_count is still 0, route to one retry.
    Otherwise, finish with the current results.
    """
    products = state.get("products", [])
    structured_query = state.get("structured_query", {})
    display_top_k = int(state.get("display_top_k", 6))
    retry_count = int(state.get("retry_count", 0))

    quality_check = _evaluate_top_products(
        products=products,
        structured_query=structured_query,
        display_top_k=display_top_k,
    )

    should_retry = (not quality_check["is_good_enough"]) and retry_count < 1
    quality_check["should_retry"] = should_retry
    quality_check["retry_count"] = retry_count

    return {
        **state,
        "quality_check": quality_check,
        "retry_reason": quality_check.get("reason"),
        "mode": "quality_checked",
        "debug_trace": _add_trace(
            state,
...
def _build_retry_search_text(state: FashionState) -> str:
    """
    Build a broader retry query from the structured query.

    The retry keeps important attributes but drops overly specific details first.
    This helps recover when the first retrieval is too narrow.
    """
    structured_query = state.get("structured_query", {})
    original_query = state.get("user_query") or ""
    search_description = state.get("search_description") or original_query
    ...
    # If no structured attributes were extracted, retry with the raw user text.
    # If that is empty or too vague, keep the generated search description.
    return original_query.strip() or search_description.strip()


def rewrite_query_and_retry_node(state: FashionState) -> FashionState:
    """
    Step 7: rewrite the query once and send the graph through retrieval again.

    This is a controlled fallback, not an endless loop. The conditional edge only
    allows retry when retry_count < 1.
    """
    retry_count = int(state.get("retry_count", 0)) + 1
    retry_search_text = _build_retry_search_text(state)
    rewritten_structured_query = _extract_structured_query(retry_search_text)

    has_image = bool(state.get("has_image"))
    has_text = bool(retry_search_text)

    if has_image and has_text:
        retry_strategy = "image_priority_multimodal"
    elif has_image:
        retry_strategy = "image_only"
    elif has_text:
        retry_strategy = "text_only"
    else:
        retry_strategy = "none"

    retry_alpha = _select_dynamic_alpha(retry_strategy)

    return {
        **state,
        "retry_count": retry_count,
        "search_description": retry_search_text,
        "structured_query": rewritten_structured_query,
        ...
        "debug_trace": _add_trace(
            ...
            "rewrite_query_and_retry",
            {
                ...
                "retry_search_text": retry_search_text,
                "retry_strategy": retry_strategy,
                "retry_alpha": retry_alpha,
                "rewritten_structured_query": rewritten_structured_query,
            },
        ),
    }


def route_after_quality_check(state: FashionState) -> str:
    """Conditional routing after Step 7 quality evaluation."""
    quality_check = state.get("quality_check", {})
    if quality_check.get("should_retry"):
        return "retry"
    return "finish"
...
def build_fashion_graph():
    ...
    graph.add_node("evaluate_result_quality", evaluate_result_quality_node)
    graph.add_node("rewrite_query_and_retry", rewrite_query_and_retry_node)
    ...
    graph.add_edge("rerank_candidates", "evaluate_result_quality")
    graph.add_conditional_edges(
        "evaluate_result_quality",
        route_after_quality_check,
        {
            "retry": "rewrite_query_and_retry",
            "finish": "update_conversation",
        },
    )
    graph.add_edge("rewrite_query_and_retry", "retrieve_image_candidates")
